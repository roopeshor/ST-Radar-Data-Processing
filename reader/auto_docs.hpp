...
constexpr const char* DOC_baudLength_us = "If the pulse is phase-coded (compressed), the pulse is divided into smaller \"bauds\" or \"chips\". The baud length dictates the actual range resolution (ΔR=c⋅baud/2), while the longer total pulse width maintains high average power.";
...
constexpr const char* DOC_phaseCodingEnabled = "A boolean/integer flag indicating whether phase coding (pulse compression) is enabled. Formally: m_sCodeFlag";
constexpr const char* DOC_IIP_us = "(IPP): Inter-Pulse Period. The time between consecutive transmitted pulses. It determines the maximum unambiguous range (Rmax​=c⋅IPP/2) and the maximum measurable Doppler velocity (Nyquist limit). Formally: m_fIntrPulsePeriod_us";
constexpr const char* DOC_pulseWidth_us = "The total duration of the transmitted RF pulse. Dictates the total transmitted energy. formally:  m_fPulseWidth_us";
...
constexpr const char* DOC_TRP_RF_Delay_us = "Transmit/Receive Path Delay. The internal hardware time it takes for the signal to travel through cables, T/R switches, and filters. This must be subtracted from the time-of-flight to accurately calculate \"Altitude Zero\".";
